...
class Validator
{
    /*stores the data from the $_POST / GET Super global*/
    protected $data = [];

    /* Registers the validation methoods*/ 
    ...
    /*Stores error messages generated by the validator */                                
    protected $errors = [];


    /*
     Constructor that takes the $Post superglobal and initialzes the $data property  
    */
    ...
    {
        if (isset($_POST['validation_done_by_js']) && (!$_POST['validation_done_by_js']))){
            return true;
        }
    ...
    /* The validator for the national if Field*/
    ...
    protected function isLicenseNumValid(string $licenseno): bool{
        if(empty($licenseno))
        {
            $this->setErrors('license_num', 'Invalid license number length');
            return false;
        }
        ...
        {
            $this->setErrors('license_num', 'License number must contain only digits');
            return false;
        }

        //validating the date
        $late_eight= substr($licenseno, 7);
        $yyyy =substr($last_eight, 0, 4);
        $mm =substr($last_eight, 4 , 2);
        $dd= substr($last_eight, 6 ,2);

        if(!checkdate($mm, $dd, $yyyy))
        {
            $this->setErrors('license_num', 'Invalid date')
            return false;
        }
        $today = date_create();
        $license_date= date_create("$yyyy-$mm-$dd");
        $interval =date_create($today, $license_date);
